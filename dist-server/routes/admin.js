...
router.get('/stats', auth_1.staffMiddleware, (req, res) => {
  ...
  `).all();
    const pendingShipmentOrders = index_1.sqlite.prepare(`
...
router.get('/staff', auth_1.adminMiddleware, (req, res) => {
    const staffList = index_1.sqlite.prepare(`SELECT id, email, name, phone, level, permissions, is_active, pro_test_limit, created_at FROM users WHERE level IN ('admin','staff') ORDER BY created_at DESC`).all();
    // 解析 permissions JSON
    staffList.forEach((s) => {
        if (s.permissions) {
            try {
...
router.post('/staff', auth_1.adminMiddleware, async (req, res) => {
    try {
        ...
        const r = index_1.sqlite.prepare(`INSERT INTO users (email,password,name,phone,level,permissions,pro_test_limit,is_active,created_at,updated_at) VALUES (?,?,?,?,?,?,?,1,?,?)`).run(email, hashed, name, null, 'staff', permsStr, limitNum, now, now);
...
router.put('/staff/:id', auth_1.adminMiddleware, (req, res) => {
    ...
    if (proTestLimit !== undefined) {
        const limitNum = parseInt(proTestLimit) || 0;
        index_1.sqlite.prepare('UPDATE users SET pro_test_limit=?, updated_at=? WHERE id=?').run(limitNum, now, req.params.id);
    }
...
// 修改员工深度测肤上限
router.put('/staff/:id/pro-test-limit', auth_1.adminMiddleware, (req, res) => {
    ...
    index_1.sqlite.prepare('UPDATE users SET pro_test_limit=?, updated_at=? WHERE id=?').run(limitNum, Date.now(), req.params.id);
    res.json({ success: true });
});
// ========== 用户管理（需 users 权限） ==========
router.get('/users', (0, auth_1.permissionMiddleware)('users'), (req, res) => {
    ...
    const users = index_1.sqlite.prepare(`SELECT u.id, u.email, u.name, u.phone, u.level, u.points, u.total_spend, u.is_active, u.created_at, u.referral_code, u.partner_tier, u.pro_test_limit, 
...
// 修改合伙人等级
router.put('/users/:id/partner-tier', (0, auth_1.permissionMiddleware)('users'), (req, res) => {
    ...
    index_1.sqlite.prepare('UPDATE users SET partner_tier=?,updated_at=? WHERE id=? AND level NOT IN (?,?)').run(partnerTier || null, Date.now(), req.params.id, 'admin', 'staff');
    res.json({ success: true });
});
// 修改深度测肤上限
router.put('/users/:id/pro-test-limit', (0, auth_1.permissionMiddleware)('users'), (req, res) => {
    const { proTestLimit } = req.body;
    const limitNum = parseInt(proTestLimit);
    if (isNaN(limitNum) || limitNum < 0) {
        res.status(400).json({ error: '无效的次数上限' });
    ...
    index_1.sqlite.prepare('UPDATE users SET pro_test_limit=?,updated_at=? WHERE id=? AND level NOT IN (?,?)').run(limitNum, Date.now(), req.params.id, 'admin', 'staff');
    res.json({ success: true });
});
// 禁用/启用用户
...
router.get('/categories', (0, auth_1.permissionMiddleware)('products'), (req, res) => {
    const categories = index_1.sqlite.prepare('SELECT * FROM categories ORDER BY sort_order').all();
    res.json({ categories });
});
router.post('/categories', (0, auth_1.permissionMiddleware)('products'), (req, res) => {
    const { name, slug, description, image, sort_order, is_active, translations } = req.body;
    if (!name || !slug)
        return res.status(400).json({ error: '分类名称和别名必填' });
...
router.put('/categories/:id', (0, auth_1.permissionMiddleware)('products'), (req, res) => {
    const { name, slug, description, image, sort_order, is_active, translations } = req.body;
    if (!name || !slug)
        return res.status(400).json({ error: '分类名称和别名必填' });
    const transStr = translations ? (typeof translations === 'string' ? translations : JSON.stringify(translations)) : null;
...
router.delete('/categories/:id', (0, auth_1.permissionMiddleware)('products'), (req, res) => {
    // Check if any products are using this category
    const productsCount = index_1.sqlite.prepare('SELECT COUNT(*) as c FROM products WHERE category_id=?').get(req.params.id).c;
    if (productsCount > 0) {
        res.status(400).json({ error: '该分类下还有商品，无法删除' });
        return;
    }
    index_1.sqlite.prepare('DELETE FROM categories WHERE id=?').run(req.params.id);
    res.json({ success: true });
});
// ========== 通用标签分类 CRUD (dosage_forms / skin_concerns / skin_types) ==========
const TAG_TABLES = ['dosage_forms', 'skin_concerns', 'skin_types'];
for (const tableName of TAG_TABLES) {
    const route = tableName.replace(/_/g, '-'); // dosage-forms, skin-concerns, skin-types
    router.get(`/${route}`, (0, auth_1.permissionMiddleware)('products'), (req, res) => {
        const items = index_1.sqlite.prepare(`SELECT * FROM ${tableName} ORDER BY sort_order`).all();
        res.json({ items });
    });
    router.post(`/${route}`, (0, auth_1.permissionMiddleware)('products'), (req, res) => {
        const { name, slug, sort_order = 0, is_active = 1, translations } = req.body;
        try {
            index_1.sqlite.prepare(`INSERT INTO ${tableName} (name, slug, sort_order, is_active, translations) VALUES (?,?,?,?,?)`).run(name, slug, sort_order || 0, is_active ?? 1, translations ? JSON.stringify(translations) : null);
            res.json({ success: true });
        }
        catch (err) {
    ...
    router.put(`/${route}/:id`, (0, auth_1.permissionMiddleware)('products'), (req, res) => {
        const { name, slug, sort_order = 0, is_active = 1, translations } = req.body;
        try {
            index_1.sqlite.prepare(`UPDATE ${tableName} SET name=?, slug=?, sort_order=?, is_active=?, translations=? WHERE id=?`).run(name, slug, sort_order || 0, is_active ?? 1, translations ? JSON.stringify(translations) : null, req.params.id);
            res.json({ success: true });
        }
        catch (err) {
            if (err.message.includes('UNIQUE constraint failed')) {
    ...
    router.delete(`/${route}/:id`, (0, auth_1.permissionMiddleware)('products'), (req, res) => {
        index_1.sqlite.prepare(`DELETE FROM ${tableName} WHERE id=?`).run(req.params.id);
        res.json({ success: true });
    });
}
...
router.post('/products', (0, auth_1.permissionMiddleware)('products'), (req, res) => {
    ...
    const transStr = translations ? (typeof translations === 'string' ? translations : JSON.stringify(translations)) : null;
    const r = index_1.sqlite.prepare(`INSERT INTO products (name,slug,description,detail,category_id,base_price,points_price,stock,unit,main_image,images,is_active,sort_order,is_featured,tags,is_bundle,skin_types,concerns,dosage_forms,before_after_images,is_sample,is_story_page,translations,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,1,?,?,?,?,?,?,?,?,?,?,?,?,?)`).run(name, slug || name.replace(/\s+/g, '-'), description, detail || null, categoryId, basePrice, pointsPrice || null, stock || 0, unit || '件', mainImage || null, imagesStr, sortOrder || 0, featuredFlag, tagsStr, bundleFlag, skinTypesStr, concernsStr, dosageFormsStr, beforeAfterStr, sampleFlag, storyFlag, transStr, now, now);
    ...
    if (prices) {
        ...
        // 默认折扣
        const defaultDiscounts = { guest: 1.0, member: 0.9, silver: 0.8, gold: 0.7, diamond: 0.6 };
...
router.put('/products/:id', (0, auth_1.permissionMiddleware)('products'), (req, res) => {
    ...
    const transStr = translations ? (typeof translations === 'string' ? translations : JSON.stringify(translations)) : null;
    const now = Date.now();
    index_1.sqlite.prepare(`UPDATE products SET name=?,description=?,detail=?,category_id=?,base_price=?,points_price=?,stock=?,unit=?,main_image=?,images=?,is_active=?,sort_order=?,is_featured=?,tags=?,is_bundle=?,skin_types=?,concerns=?,dosage_forms=?,before_after_images=?,is_sample=?,is_story_page=?,translations=?,updated_at=? WHERE id=?`).run(name, description, detail || null, categoryId, basePrice, pointsPrice || null, stock, unit, mainImage || null, imagesStr, isActive ? 1 : 0, sortOrder || 0, featuredFlag, tagsStr, bundleFlag, skinTypesStr, concernsStr, dosageFormsStr, beforeAfterStr, sampleFlag, storyFlag, transStr, now, req.params.id);
    if (isBundle && bundleProductIds && Array.isArray(bundleProductIds)) {
...
// 快捷编辑商品排序与推荐状态
router.patch('/products/:id/quick-edit', (0, auth_1.permissionMiddleware)('products'), (req, res) => {
    ...
    if (updates.length > 0) {
        ...
        index_1.sqlite.prepare(`UPDATE products SET ${updates.join(', ')}, updated_at = ? WHERE id = ?`).run(...params);
    }
    res.json({ success: true });
});
// 删除商品
...
// 订单发货
router.put('/orders/:id/ship', (0, auth_1.permissionMiddleware)('orders'), (req, res) => {
...
// 删除订单（仅限已取消）
router.delete('/orders/:id', (0, auth_1.permissionMiddleware)('orders'), (req, res) => {
    const orderId = req.params.id;
    const order = index_1.sqlite.prepare('SELECT status FROM orders WHERE id=?').get(orderId);
    if (!order) {
    ...
    const deleteTx = index_1.sqlite.transaction(() => {
        index_1.sqlite.prepare('DELETE FROM order_items WHERE order_id=?').run(orderId);
        index_1.sqlite.prepare('DELETE FROM payment_logs WHERE order_id=?').run(orderId);
        index_1.sqlite.prepare('DELETE FROM reviews WHERE order_id=?').run(orderId);
        index_1.sqlite.prepare('DELETE FROM orders WHERE id=?').run(orderId);
    });
    try {
        deleteTx();
        res.json({ success: true });
...
router.get('/settings', (0, auth_1.permissionMiddleware)('settings', 'marketing', 'articles', 'ai'), (_req, res) => {
    const settings = index_1.sqlite.prepare('SELECT key, value, description, translations FROM site_settings ORDER BY id').all();
    const map = {};
    settings.forEach(s => {
        map[s.key] = s.value;
    });

...
请尽量用简短、自然、像真人一样的口吻聊天，不要长篇大论。`;
        ...
        const modifiedLogs = index_1.sqlite.prepare(`SELECT question, answer FROM ai_chat_logs WHERE is_modified = 1 ORDER BY updated_at DESC LIMIT 20`).all();
...
// 获取聊天记录
router.get('/chat-logs', auth_1.authMiddleware, (0, auth_1.permissionMiddleware)('ai'), (req, res) => {
    try {
        ...
        const logs = index_1.sqlite.prepare(query).all(...params);
        res.json(logs);
    }
    catch (e) {
...
// 修改聊天记录的回复
router.put('/chat-logs/:id', auth_1.authMiddleware, (0, auth_1.permissionMiddleware)('ai'), (req, res) => {
    try {
        ...
        index_1.sqlite.prepare('UPDATE ai_chat_logs SET answer = ?, is_modified = 1, updated_at = ? WHERE id = ?').run(answer, Date.now(), req.params.id);
...
router.post('/translate', auth_1.authMiddleware, (0, auth_1.permissionMiddleware)('admin', 'content', 'product', 'articles'), async (req, res) => {
    try {
...
router.post('/seo-optimize', auth_1.authMiddleware, (0, auth_1.permissionMiddleware)('manage_marketing'), async (req, res) => {
    try {
...
router.post('/generate-keyword-pool', auth_1.authMiddleware, (0, auth_1.permissionMiddleware)('manage_marketing', 'settings', 'articles', 'products'), async (req, res) => {
    try {
        const getSetting = (key) => {
            const row = index_1.sqlite.prepare(`SELECT value FROM site_settings WHERE key = ?`).get(key);
            return row?.value;
...
// 获取旷视/百度皮肤分析历史记录
router.get('/skin-records', auth_1.authMiddleware, (0, auth_1.permissionMiddleware)('ai'), (req, res) => {
    try {
        const records = index_1.sqlite.prepare(`
      SELECT s.id, s.image_url, s.result_data, s.created_at, u.name as user_name, u.email as user_email

...
async function initDB() {
  ...
  `);
    ...
    try {
        ...
        const insertSetting = index_1.sqlite.prepare('INSERT OR IGNORE INTO site_settings (key, value, description, updated_at) VALUES (?,?,?,?)');
        const now = Date.now();
    ...
    try {
        index_1.sqlite.exec(`ALTER TABLE products ADD COLUMN detail TEXT`);
        console.log('[DB] Column detail added to products');
    }
    catch (e) {
        if (!e.message.includes('duplicate column'))
            console.log('[DB] detail column already exists or skipped:', e.message);
    }
    ...
    for (const table of tablesWithTranslations) {
        try {
            index_1.sqlite.exec(`ALTER TABLE ${table} ADD COLUMN translations TEXT`);
            console.log(`[DB] Column translations added to ${table}`);
        }
        catch (e) {
            if (!e.message.includes('duplicate column'))
                console.log(`[DB] translations column already exists in ${table} or skipped:`, e.message);
        }
    }
    // 迁移：给 users 表添加 permissions 列
    try {
        index_1.sqlite.exec(`ALTER TABLE users ADD COLUMN permissions TEXT`);
        console.log('[DB] Column permissions added to users');
    }
    catch (e) {
        if (!e.message.includes('duplicate column'))
            console.log('[DB] permissions column already exists or skipped:', e.message);
    }
    // 迁移：给 users 表添加 partner_tier 列
    try {
        index_1.sqlite.exec(`ALTER TABLE users ADD COLUMN partner_tier TEXT`);
        console.log('[DB] Column partner_tier added to users');
    }
    catch (e) {
        if (!e.message.includes('duplicate column'))
            console.log('[DB] partner_tier column already exists or skipped:', e.message);
    }
    // 迁移：给 users 表添加 pro_test_limit 列
    try {
        index_1.sqlite.exec(`ALTER TABLE users ADD COLUMN pro_test_limit INTEGER NOT NULL DEFAULT 4`);
        console.log('[DB] Column pro_test_limit added to users');
    }
    catch (e) {
        if (!e.message.includes('duplicate column'))
            console.log('[DB] pro_test_limit column skipped:', e.message);
    }
    // 迁移：给 skin_analysis_records 表添加 type 列
    try {
        index_1.sqlite.exec(`ALTER TABLE skin_analysis_records ADD COLUMN type TEXT NOT NULL DEFAULT 'basic'`);
        console.log('[DB] Column type added to skin_analysis_records');
    }
    catch (e) {
        if (!e.message.includes('duplicate column'))
            console.log('[DB] type column skipped:', e.message);
    }
    ...
    try {
        index_1.sqlite.exec(`ALTER TABLE products ADD COLUMN is_bundle INTEGER NOT NULL DEFAULT 0`);
        console.log('[DB] Column is_bundle added to products');
    }
    catch (e) {
        if (!e.message.includes('duplicate column'))
            console.log('[DB] is_bundle column already exists or skipped:', e.message);
    }
    try {
        index_1.sqlite.exec(`
    ...
    try {
        ...
        const testInsert = index_1.sqlite.prepare("SELECT sql FROM sqlite_master WHERE type='table' AND name='users'").get();
        if (testInsert && testInsert.sql && testInsert.sql.includes("'staff'")) {
            console.log('[DB] Users table already supports staff level');
        }
        else {
            // 需要重建表，先禁用外键约束
            index_1.sqlite.exec('PRAGMA foreign_keys = OFF');
            index_1.sqlite.exec(`
      ...
      `);
            index_1.sqlite.exec('PRAGMA foreign_keys = ON');
    ...
    try {
        const testInsert = index_1.sqlite.prepare("SELECT sql FROM sqlite_master WHERE type='table' AND name='product_prices'").get();
        if (testInsert && testInsert.sql && testInsert.sql.includes("'diamond'")) {
      ...
      `);
            ...
            const missingPrices = index_1.sqlite.prepare(`
      ...
      `).all();
            const insertDiamond = index_1.sqlite.prepare(`INSERT INTO product_prices (product_id, level, price, discount) VALUES (?, 'diamond', ?, 0.6)`);
    ...
    try {
        ...
        const insertPaymentSetting = index_1.sqlite.prepare('INSERT OR IGNORE INTO site_settings (key, value, description, updated_at) VALUES (?,?,?,?)');
    ...
    // 迁移：积分与推荐系统相关字段和表
    try {
        index_1.sqlite.exec(`ALTER TABLE users ADD COLUMN referral_code TEXT`);
        console.log('[DB] Column referral_code added to users');
    }
    catch (e) {
        if (!e.message.includes('duplicate column'))
            console.log('[DB] referral_code column skipped:', e.message);
    }
    try {
        index_1.sqlite.exec(`CREATE UNIQUE INDEX IF NOT EXISTS idx_users_referral_code ON users(referral_code);`);
    }
    catch (e) {
        console.log('[DB] referral_code index skipped:', e.message);
    }
    try {
        index_1.sqlite.exec(`ALTER TABLE users ADD COLUMN referred_by INTEGER`);
        console.log('[DB] Column referred_by added to users');
    }
    catch (e) {
        if (!e.message.includes('duplicate column'))
            console.log('[DB] referred_by column skipped:', e.message);
    }
    try {
        index_1.sqlite.exec(`ALTER TABLE products ADD COLUMN points_price INTEGER`);
        console.log('[DB] Column points_price added to products');
    }
    catch (e) {
        if (!e.message.includes('duplicate column'))
            console.log('[DB] points_price column skipped:', e.message);
    }
    try {
        index_1.sqlite.exec(`
    ...
    for (const col of productColumns) {
        try {
            index_1.sqlite.exec(`ALTER TABLE products ADD COLUMN ${col} TEXT`);
            console.log(`[DB] Column ${col} added to products`);
        }
        catch (e) {
            if (!e.message.includes('duplicate column'))
                console.log(`[DB] ${col} column skipped:`, e.message);
        }
    }
    const phase2ProductColumns = ['is_sample', 'is_story_page', 'is_featured'];
    for (const col of phase2ProductColumns) {
        try {
            index_1.sqlite.exec(`ALTER TABLE products ADD COLUMN ${col} INTEGER NOT NULL DEFAULT 0`);
            console.log(`[DB] Column ${col} added to products`);
        }
        catch (e) {
            if (!e.message.includes('duplicate column'))
    ...
    try {
        index_1.sqlite.exec(`ALTER TABLE orders ADD COLUMN is_gift INTEGER NOT NULL DEFAULT 0`);
        index_1.sqlite.exec(`ALTER TABLE orders ADD COLUMN gift_message TEXT`);
        index_1.sqlite.exec(`ALTER TABLE orders ADD COLUMN gift_wrap_fee REAL NOT NULL DEFAULT 0`);
        console.log('[DB] Gifting columns added to orders');
    }
    catch (e) {
        if (!e.message.includes('duplicate column'))
            console.log('[DB] Gifting columns skipped:', e.message);
    }
    // 迁移：给 orders 表添加物流相关字段
    try {
        index_1.sqlite.exec(`ALTER TABLE orders ADD COLUMN tracking_company TEXT`);
        index_1.sqlite.exec(`ALTER TABLE orders ADD COLUMN tracking_number TEXT`);
        console.log('[DB] Tracking columns added to orders');
    }
    catch (e) {
        if (!e.message.includes('duplicate column'))
            console.log('[DB] Tracking columns skipped:', e.message);
    }
    await seedData();
    seedSiteSettings();
}
async function seedData() {
    // 检查是否已经有数据
    const adminExists = index_1.sqlite.prepare('SELECT id FROM users WHERE level = ? LIMIT 1').get('admin');
    if (adminExists)
        return;
    console.log('[DB] Seeding initial data...');
    const now = Date.now();
    // 创建管理员
    const adminPwd = await bcryptjs_1.default.hash('admin123', 10);
    index_1.sqlite.prepare(`INSERT INTO users (email,password,name,level,is_active,created_at,updated_at) VALUES (?,?,?,?,?,?,?)`).run('admin@skincare.com', adminPwd, '管理员', 'admin', 1, now, now);
    // 创建测试用户
    const testPwd = await bcryptjs_1.default.hash('test123', 10);
    const memberUsers = [
    ...
    for (const u of memberUsers) {
        index_1.sqlite.prepare(`INSERT INTO users (email,password,name,level,is_active,created_at,updated_at) VALUES (?,?,?,?,?,?,?)`).run(u.email, testPwd, u.name, u.level, 1, now, now);
    }
    // 创建分类
    const categories = [
    ...
    for (let i = 0; i < categories.length; i++) {
        ...
        index_1.sqlite.prepare(`INSERT INTO categories (name,slug,description,sort_order,is_active) VALUES (?,?,?,?,1)`).run(c.name, c.slug, c.description, i);
    }
    // 创建示例商品
    ...
    const catMap = {};
    const catRows = index_1.sqlite.prepare('SELECT id, slug FROM categories').all();
    ...
    for (const p of sampleProducts) {
        const r = index_1.sqlite.prepare(`INSERT INTO products (category_id,name,slug,description,base_price,stock,is_active,created_at,updated_at) VALUES (?,?,?,?,?,?,1,?,?)`).run(catMap[p.catSlug], p.name, p.slug, p.desc, p.basePrice, p.stock, now, now);
        ...
        for (const [level, discount] of Object.entries(levelDiscounts)) {
            ...
            index_1.sqlite.prepare(`INSERT INTO product_prices (product_id,level,price,discount) VALUES (?,?,?,?)`).run(productId, level, price, discount);
        }
    }
...
function seedSiteSettings() {
    ...
    const insert = index_1.sqlite.prepare(`

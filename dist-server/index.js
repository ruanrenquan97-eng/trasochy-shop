...
const clinical_reports_1 = __importDefault(require("./routes/clinical-reports"));
const payment_1 = __importDefault(require("./routes/payment"));
const skin_1 = __importDefault(require("./routes/skin"));
...
app.use('/api/clinical-reports', clinical_reports_1.default);
app.use('/api/skin', skin_1.default);
// 健康检查
...
app.get('/api/settings', (req, res) => {
    ...
    let settings = [];
    try {
        settings = index_1.sqlite.prepare('SELECT key, value, translations FROM site_settings').all();
    }
    catch (e) {
        // Fallback if translations column doesn't exist yet
        settings = index_1.sqlite.prepare('SELECT key, value FROM site_settings').all();
    }
    const map = {};
    settings.forEach(s => {
...
app.get('/sitemap.xml', (_req, res) => {
    ...
    // 动态文章页面
    const articles = index_1.sqlite.prepare('SELECT slug, updated_at, published_at FROM articles WHERE is_published = 1').all();

...
exports.analyzeSkinWithMegvii = analyzeSkinWithMegvii;
 ...
 * @returns 皮肤分析结果 JSON
 */
async function analyzeSkinWithMegvii(imageBase64, dynamicApiKey, dynamicApiSecret, isPro = false, apiUrl) {
    ...
    while (attempt < MAX_RETRIES) {
        ...
        if (isPro) {
            ...
            if (data.result) {
                ...
                if (data.result.maps)
                    console.log('[Megvii Pro] maps keys:', Object.keys(data.result.maps));
            }
            if (data.maps)
                console.log('[Megvii Pro] data.maps keys:', Object.keys(data.maps));
        }

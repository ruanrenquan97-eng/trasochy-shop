...
if (row) {
  ...
  if (len >= 2) {
    innovationMembers[len - 2].img = '/images/european_female_researcher_1_logo.png';
    innovationMembers[len - 1].img = '/images/european_female_researcher_3_logo.png';
  }
  db.prepare(`UPDATE site_settings SET value = ? WHERE key='brand_team_members'`).run(JSON.stringify(members));
  console.log('Successfully updated images with logos');
}

...
// 获取公开标签 (肤质/诉求/剂型)
router.get('/tags/:type', (req: Request, res: Response) => {
  ...
  if (!tableName) return res.status(400).json({ error: 'Invalid tag type' });

  const items = sqlite.prepare(`SELECT * FROM ${tableName} WHERE is_active = 1 ORDER BY sort_order`).all() as any[];

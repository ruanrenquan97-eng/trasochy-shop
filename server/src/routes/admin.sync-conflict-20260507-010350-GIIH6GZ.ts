...
router.get('/staff', adminMiddleware, (req: Request, res: Response) => {
  const staffList = sqlite.prepare(
    `SELECT id, email, name, phone, level, permissions, is_active, created_at FROM users WHERE level IN ('admin','staff') ORDER BY created_at DESC`
  ).all() as any[];
  // 解析 permissions JSON
...
router.post('/staff', adminMiddleware, async (req: Request, res: Response) => {
  try {
    ...
    const r = sqlite.prepare(
      `INSERT INTO users (email,password,name,phone,level,permissions,is_active,created_at,updated_at) VALUES (?,?,?,?,?,?,1,?,?)`
    ).run(email, hashed, name, null, 'staff', permsStr, now, now);
...
router.get('/users', permissionMiddleware('users'), (req: Request, res: Response) => {
  ...
  const users = sqlite.prepare(
    `SELECT u.id, u.email, u.name, u.phone, u.level, u.points, u.total_spend, u.is_active, u.created_at, u.referral_code, 
...
router.post('/products', permissionMiddleware('products'), (req: Request, res: Response) => {
  ...
  const r = sqlite.prepare(`INSERT INTO products (name,slug,description,detail,category_id,base_price,points_price,stock,unit,main_image,images,is_active,sort_order,tags,is_bundle,skin_types,concerns,before_after_images,is_sample,is_story_page,translations,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,1,?,?,?,?,?,?,?,?,?,?,?)`).run(
    name, slug || name.replace(/\s+/g, '-'), description, detail || null, categoryId, basePrice, pointsPrice || null, stock || 0, unit || '件', mainImage || null, imagesStr, sortOrder || 0, tagsStr, bundleFlag, skinTypesStr, concernsStr, beforeAfterStr, sampleFlag, storyFlag, transStr, now, now
  );
  ...
  if (prices) {
    ...
    // 默认折扣
    const discounts: Record<string, number> = { guest: 1.0, member: 0.9, silver: 0.8, gold: 0.7 };
...
router.put('/products/:id', permissionMiddleware('products'), (req: Request, res: Response) => {
  ...
  const transStr = translations ? (typeof translations === 'string' ? translations : JSON.stringify(translations)) : null;
  const now = Date.now();
  sqlite.prepare(`UPDATE products SET name=?,description=?,detail=?,category_id=?,base_price=?,points_price=?,stock=?,unit=?,main_image=?,images=?,is_active=?,sort_order=?,tags=?,is_bundle=?,skin_types=?,concerns=?,before_after_images=?,is_sample=?,is_story_page=?,translations=?,updated_at=? WHERE id=?`).run(
    name, description, detail || null, categoryId, basePrice, pointsPrice || null, stock, unit, mainImage || null, imagesStr, isActive ? 1 : 0, sortOrder || 0, tagsStr, bundleFlag, skinTypesStr, concernsStr, beforeAfterStr, sampleFlag, storyFlag, transStr, now, req.params.id
  );
...
router.get('/settings', permissionMiddleware('settings', 'marketing', 'articles', 'ai'), (_req: Request, res: Response) => {
  const settings = sqlite.prepare('SELECT key, value, description FROM site_settings ORDER BY id').all() as any[];

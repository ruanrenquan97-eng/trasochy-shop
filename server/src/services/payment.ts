...
import * as StripePay from './stripe';
...
export interface CreatePaymentParams {
  orderNo: string;
  channel: 'wechat' | 'alipay' | 'visa' | 'paypal';
  clientIp?: string;
  userAgent?: string;
  returnUrl: string;
}
...
export interface CreatePaymentResult {
  type: 'h5_url' | 'jsapi' | 'form' | 'mock' | 'native_qrcode';
  data: any;
  orderNo: string;
  payAmount: number;
}
...
export function processNotification(
  ...
  const updateOrder = sqlite.transaction(() => {
    ...
    // 读取合伙人返利比例配置
    const rebateDefaultRow = sqlite.prepare("SELECT value FROM site_settings WHERE key='partner_rebate_default'").get() as any;
    const rebateAdvancedRow = sqlite.prepare("SELECT value FROM site_settings WHERE key='partner_rebate_advanced'").get() as any;
    const rebateSuperRow = sqlite.prepare("SELECT value FROM site_settings WHERE key='partner_rebate_super'").get() as any;
    const rebateGoldRow = sqlite.prepare("SELECT value FROM site_settings WHERE key='partner_rebate_gold'").get() as any;
    const rebateDiamondRow = sqlite.prepare("SELECT value FROM site_settings WHERE key='partner_rebate_diamond'").get() as any;
    ...
    // 读取合伙人邀请门槛配置
    const thresholdAdvRow = sqlite.prepare("SELECT value FROM site_settings WHERE key='partner_threshold_advanced'").get() as any;
    const thresholdSupRow = sqlite.prepare("SELECT value FROM site_settings WHERE key='partner_threshold_super'").get() as any;
    const thresholdGoldRow = sqlite.prepare("SELECT value FROM site_settings WHERE key='partner_threshold_gold'").get() as any;
    const thresholdDiaRow = sqlite.prepare("SELECT value FROM site_settings WHERE key='partner_threshold_diamond'").get() as any;
    ...
    if (buyer && buyer.referred_by) {
      const referrer = sqlite.prepare("SELECT partner_tier FROM users WHERE id=?").get(buyer.referred_by) as any;
      
      let referralPercentage = rebateDefault;
      if (referrer && referrer.partner_tier) {
        // 使用手动覆盖的等级
        if (referrer.partner_tier === 'diamond') referralPercentage = rebateDiamond;
        else if (referrer.partner_tier === 'gold') referralPercentage = rebateGold;
        else if (referrer.partner_tier === 'super') referralPercentage = rebateSuper;
        else if (referrer.partner_tier === 'advanced') referralPercentage = rebateAdvanced;
      } else {
        // 自动计算推荐人已推荐的总人数
        ...
        if (refCount >= tDia) referralPercentage = rebateDiamond;
        else if (refCount >= tGold) referralPercentage = rebateGold;
        else if (refCount >= tSup) referralPercentage = rebateSuper;
        else if (refCount >= tAdv) referralPercentage = rebateAdvanced;
      }

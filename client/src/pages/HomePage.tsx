...
export default function HomePage() {
  ...
  const { data: productsData } = useQuery({
    ...
    queryFn: () => api.get('/products?isFeatured=1&limit=8&sort=sort_order'),
  }) as any;
  ...
  return (
    <div className="overflow-hidden">
            ...
            <div>
              ...
              <div className="animate-fade-up-delay-3 mt-14 flex items-center gap-8 text-stone-500 text-[11px] tracking-wider">
                <div className="flex items-center gap-2">
                  ...
                  <span>{t('auto_homepage_124', t('auto_homepage_124', '正品保障'))}</span>
                </div>
                <div className="flex items-center gap-2">
                  <Leaf size={14} className="text-amber-500/70" />
                  <span>{t('auto_homepage_152', t('auto_homepage_152', '天然成分'))}</span>
                </div>
                <div className="flex items-center gap-2">
                  <Star size={14} className="text-amber-500/70" />
                  <span>{t('auto_homepage_126', t('auto_homepage_126', '万人好评'))}</span>
                </div>
              </div>
            </div>
            ...
            <div className="hidden md:block relative">
              <div className="animate-scale-in relative w-full aspect-square max-w-lg ml-auto">
                <img
                  ...
                  alt={settings.site_name || t('auto_homepage_150', '传诗奇')}
                ...
                <div className="absolute -top-4 -right-4 glass rounded-lg px-4 py-3">
                  <p className="text-[10px] text-stone-400 tracking-wider uppercase">{t('auto_homepage_129', t('auto_homepage_129', '用户满意度'))}</p>
        ...
        <div className="animate-marquee whitespace-nowrap flex">
          {[...Array(2)].map((_, i) => (
            <div key={i} className="flex items-center gap-8 mx-4">
              {[t('auto_homepage_151', '科学配方'), t('auto_homepage_152', '天然成分'), t('auto_homepage_153', '高端品质'), t('auto_homepage_154', '焕颜修护'), t('auto_homepage_155', '温和亲肤'), t('auto_homepage_156', '品牌保障')].map(text => (
        ...
        <RevealSection>
          <section className="max-w-7xl mx-auto px-6 pb-12 md:pb-16">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
              ...
              <CategoryCard key="bundle" cat={{ id: 999, name: t('auto_homepage_165', '组合套装'), slug: 'bundle' } as any} index={categories.length > 5 ? 5 : categories.length} />
            </div>
          </section>
        </RevealSection>
      )}
      ...
      {/* ═══════ 精选商品 ═══════ */}
      <RevealSection>
        <section className="bg-stone-100/50 py-12 md:py-16 px-6">
          <div className="max-w-7xl mx-auto">
            ...
            {products.length > 0 ? (
              ...
              <div className="text-center py-20">
                ...
                <p className="text-stone-400 text-sm tracking-wider">{t('auto_homepage_130', t('auto_homepage_130', '商品正在上架中...'))}</p>
              </div>
            )}
          </div>
        </section>
            ...
            <div className="relative z-10 grid md:grid-cols-2 gap-12 p-10 md:p-16 items-center">
              <div>
                ...
                <p className="text-stone-400 text-sm leading-relaxed mb-8">
                  {settings.brand_story_text || t('auto_homepage_167', '传诗奇相信，真正的美丽源于科学的配方与自然的馈赠。我们甄选全球优质原料，融合尖端生物科技，为不同肤质量身定制护肤方案。每一款产品都经过严格测试，只为带给你安心、有效的护肤体验。')}
                </p>
                <button onClick={() => navigate('/products')} className="btn-gold btn-shimmer px-8 py-3 text-xs tracking-widest">
                  {t('auto_homepage_133', '了解更多')}
                </button>
              </div>
            ...
            <div className="relative bg-gradient-to-r from-amber-50 via-white to-rose-50 rounded-xl p-10 md:p-16 text-center border border-amber-100/50">
              ...
              <div className="relative z-10">
                ...
                <h2 className="text-2xl md:text-3xl font-light text-stone-900 tracking-tight mb-3">
                  {settings.member_cta_title || t('auto_homepage_168', '加入传诗奇，尊享会员特权')}
                </h2>
                <p className="text-stone-500 text-sm leading-relaxed max-w-lg mx-auto mb-8">
                  {settings.member_cta_text || t('auto_homepage_169', '注册即享专属折扣、积分奖励、新品优先体验等多重会员权益。不同等级解锁更多惊喜，让美丽投资更有价值。')}
                </p>
                <div className="flex flex-wrap justify-center gap-6 text-xs text-stone-600 mb-10">
                  {[
                    { level: t('auto_homepage_170', '普通会员'), color: 'bg-blue-100 text-blue-700', discount: t('auto_homepage_171', '95折') },
                    { level: t('auto_homepage_172', '银卡会员'), color: 'bg-slate-100 text-slate-600', discount: t('auto_homepage_173', '9折') },
                    { level: t('auto_homepage_174', '金卡会员'), color: 'bg-amber-100 text-amber-700', discount: t('auto_homepage_175', '85折') },
                  ].map(item => (
      ...
      <RevealSection>
        <section className="max-w-7xl mx-auto px-6 pb-12 md:pb-16">
          <div className="text-center py-12 border-t border-stone-200">
            ...
            <h2 className="text-xl font-light text-stone-800 tracking-tight mb-6">{t('auto_homepage_135', t('auto_homepage_135', '开启你的美肌之旅'))}</h2>
            <button onClick={() => navigate('/products')} className="btn-primary btn-shimmer px-10 py-3.5 text-xs tracking-widest">
              {t('auto_homepage_136', '浏览全部商品')}
            </button>
          </div>
        </section>
      </RevealSection>
            ...
            <div className="cursor-pointer" onClick={() => { closePromoModal(); navigate(settings.promo_modal_link || '/products'); }}>
              {settings.promo_modal_image ? (
                ...
                <div className="p-12 text-center bg-gradient-to-br from-amber-50 to-rose-50">
                  ...
                  <h3 className="text-2xl font-semibold text-stone-800 mb-2">{t('auto_homepage_137', t('auto_homepage_137', '限时特惠活动'))}</h3>
                  <p className="text-sm text-stone-500 mb-6">{t('auto_homepage_138', t('auto_homepage_138', '点击查看更多活动详情'))}</p>
                  <button className="btn-gold px-8 py-2.5 text-sm rounded-full shadow-lg shadow-amber-500/20">{t('auto_homepage_139', t('auto_homepage_139', '立即前往'))}</button>
                </div>
              )}
            </div>
          </div>

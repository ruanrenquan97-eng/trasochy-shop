...
export default function ArticleDetailPage() {
  ...
  if (isLoading) return <div className="text-center py-32 text-stone-400">{t('auto_staticpage_313', t('auto_staticpage_313', '加载中...'))}</div>;
      ...
      <header className="mb-12 text-center">
        {article.keywords && (
          <div className="text-xs text-stone-400 tracking-widest uppercase mb-6">
            {article.keywords.includes('[') 
              ? (() => { try { return JSON.parse(article.keywords).join(' • '); } catch(e) { return article.keywords; } })()
              : article.keywords.split(',').map((k: string) => k.trim()).filter(Boolean).join(' • ')}
          </div>
        )}

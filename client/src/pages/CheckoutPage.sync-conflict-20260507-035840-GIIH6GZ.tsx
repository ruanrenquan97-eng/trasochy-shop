...
export default function CheckoutPage() {
  ...
  const { items, total, pointsTotal } = location.state || {};
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isPointsRedemption || finalAmount === 0) {
        ...
        refreshUser();
        toast.success(isPointsRedemption ? '积分兑换成功！' : '支付成功！');
      ...
      try {
        ...
        if (!paymentResult.success) {
          toast.error(paymentResult.error || '创建支付失败');
          setSubmitting(false);
          return;
        }
        ...
        if (type === 'jsapi') {
          ...
          toast.loading('正在唤起微信支付...', { duration: 2000 });
          setSubmitting(false);
          return;
        }
        ...
        console.error('预支付创建失败:', payErr);
        toast.error('支付创建失败，请前往订单页面重试');
        navigate('/orders');
      }
    } catch (e: any) {
      toast.error(e.message || '订单创建失败');
      setSubmitting(false);
    }
  };

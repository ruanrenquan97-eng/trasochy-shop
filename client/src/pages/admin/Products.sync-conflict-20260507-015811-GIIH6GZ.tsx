...
const LEVELS = ['guest', 'member', 'silver', 'gold'];
const LEVEL_NAMES: Record<string, string> = { guest: '游客', member: '普通会员', silver: '银卡会员', gold: '金卡会员' };
...
const defaultForm = (): ProductForm => ({
  ...
  prices: { guest: { price: '', discount: '1.0' }, member: { price: '', discount: '0.9' }, silver: { price: '', discount: '0.8' }, gold: { price: '', discount: '0.7' } },
  isBundle: false, bundleProductIds: [],
  skinTypes: [], concerns: [], ingredientIds: [], beforeAfterImages: [],
  isSample: false, isStoryPage: false,
...
export default function AdminProducts() {
    ...
    onSuccess: (res: any, targetLang: string) => {
      setForm(f => ({
        ...
        translations: {
          ...f.translations,
          [targetLang]: {
            ...f.translations[targetLang],
            name: res.translated.name || f.translations[targetLang]?.name || '',
            description: res.translated.description || f.translations[targetLang]?.description || '',
            detail: res.translated.detail || f.translations[targetLang]?.detail || ''
          }
        }
      }));
      toast.success('翻译完成');
  ...
  const { data: ingredientsData } = useQuery({
    ...
    queryFn: () => api.get('/admin/ingredients'),
  }) as any;
  ...
  const handleBasePriceChange = (value: string) => {
    ...
    const defaultDiscounts: Record<string, number> = { guest: 1.0, member: 0.9, silver: 0.8, gold: 0.7 };
    setForm(f => ({
      ...f, basePrice: value,
          ...
          <table className="w-full text-sm">
            <thead className="bg-stone-50 border-b border-stone-100">
              <tr>
                ...
                <th className="text-left px-4 py-3 text-stone-500 font-medium">会员价</th>
                  ...
                  <tr key={product.id} className="hover:bg-stone-50 transition-colors">
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="font-medium text-stone-700 line-clamp-1">{product.name}</p>
                          <p className="text-xs text-stone-400">{product.slug}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-4 py-3 text-stone-500">{product.category_name || '-'}</td>
                    <td className="px-4 py-3 font-medium text-stone-700">¥{product.base_price?.toFixed(2)}</td>
                    <td className="px-4 py-3 text-xs text-stone-500">
                      <span className="bg-blue-50 text-blue-600 px-1.5 py-0.5 rounded mr-1">会员¥{memberPrice?.toFixed(2)}</span>
                      <span className="bg-amber-50 text-amber-600 px-1.5 py-0.5 rounded">金卡¥{goldPrice?.toFixed(2)}</span>
                    </td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3 flex items-center gap-2">
                      <button onClick={() => openEdit(product)} className="text-stone-400 hover:text-rose-400 transition-colors"><Edit2 size={14} /></button>
                      <button onClick={() => handleDelete(product.id)} className="text-stone-400 hover:text-red-400 transition-colors"><Trash2 size={14} /></button>
                    </td>
                  </tr>
                );
              })}
              ...
              <div className="grid grid-cols-2 gap-3 mb-4">
                ...
                <div className={`col-span-2 border border-stone-100 rounded-xl p-3 bg-stone-50 ${settings?.feature_skin_concern_filter !== '1' ? 'opacity-50 pointer-events-none' : ''}`}>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {SKIN_TYPES.map(type => (
                      <label key={type} className="flex items-center gap-1.5 bg-white px-2 py-1 border border-stone-200 rounded cursor-pointer hover:border-rose-300">
                        <input type="checkbox" checked={form.skinTypes.includes(type)} onChange={e => {
                          if (e.target.checked) setForm(f => ({ ...f, skinTypes: [...f.skinTypes, type] }));
                          else setForm(f => ({ ...f, skinTypes: f.skinTypes.filter(t => t !== type) }));
                ...
                <div className={`col-span-2 border border-stone-100 rounded-xl p-3 bg-stone-50 ${settings?.feature_skin_concern_filter !== '1' ? 'opacity-50 pointer-events-none' : ''}`}>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {CONCERNS.map(type => (
                      <label key={type} className="flex items-center gap-1.5 bg-white px-2 py-1 border border-stone-200 rounded cursor-pointer hover:border-rose-300">
                        <input type="checkbox" checked={form.concerns.includes(type)} onChange={e => {
                          if (e.target.checked) setForm(f => ({ ...f, concerns: [...f.concerns, type] }));
                          else setForm(f => ({ ...f, concerns: f.concerns.filter(t => t !== type) }));
                        }} className="w-3 h-3 text-rose-500 rounded border-stone-300" />
                        <span className="text-xs text-stone-700">{type}</span>
                      </label>
                    ))}

...
// 2. Skincare Theme Configs and Logic
function getSkincareTheme(title, keywords = []) {
...
async function createArticleCoverImage(params) {
  ...
  const titleLines = wrapSvgText(params.title || 'TRASOCHY Skin Research', 22, 3);
  const subtitleLines = wrapSvgText(params.subtitle || 'AI-powered skincare science and clinical insight', 42, 2);
  const tags = (params.keywords || []).slice(0, 4);

  // Retrieve customized theme based on title & keywords
  const theme = getSkincareTheme(params.title, params.keywords);
...
// 3. Batch Processing Main Function
async function main() {
  console.log('\n======================================================');
  console.log('  TRASOCHY SKINCARE - ARTICLE COVER UPGRADE ENGINE  ');
  console.log('======================================================\n');
  ...
  try {
    ...
    for (const art of articles) {
      ...
      // Determine if this cover should be processed:
      // - Either it has no cover image
      // - Or it has a cover image that matches the default AI SVG pattern (starts with '/uploads/articles/ai-cover-')
      const isMissingCover = !cover.trim();
      const isDefaultAiCover = cover.startsWith('/uploads/articles/ai-cover-');

      if (isMissingCover || isDefaultAiCover) {
        console.log(`\nProcessing Article #${art.id}: "${art.title}"`);
        
        let tags = [];
        if (art.keywords) {
          try {
            // Attempt to parse JSON keyword list
        ...
        const theme = getSkincareTheme(art.title, tags);
        console.log(`  -> Match theme: [${theme.themeName}] based on keywords: [${tags.join(', ')}]`);

        // Generate beautiful custom cover image
        const newCoverPath = await createArticleCoverImage({
          title: art.title,
          subtitle: tags.slice(0, 3).join(' / ') || 'TRASOCHY Advanced Skincare science',
          keywords: tags
        });
        ...
        console.log(`  -> SUCCESSFULLY UPGRADED! Old: "${cover}" => New: "${newCoverPath}"`);
        upgradedCount++;
      } else {
        console.log(`Skipping Article #${art.id}: "${art.title}" (Has custom user thumbnail: "${cover}")`);
        skippedCount++;
      }
    }
    ...
    console.log('======================================================');
    console.log(`  - Successfully Upgraded/Regenerated: ${upgradedCount} articles`);
    console.log(`  - Unchanged/Skipped (custom covers): ${skippedCount} articles`);
